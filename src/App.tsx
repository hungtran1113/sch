...
import { Calendar, ChevronLeft, ChevronRight, LogIn, UserPlus, Save, X, Users } from 'lucide-react';
...
export default function App() {
  ...
  const handleDeleteUser = async (id: string) => {
    ...
    try {
      ...
      // Tải lại lịch để làm mất các ô của người vừa bị xóa
  ...
  useEffect(() => {
    loadBookings();
    setPendingBookings([]); // clear pending when week changes
  }, [weekId]);

  // Polling for updates (since multiple users access the site)
  useEffect(() => {
  ...
  const handleWeekSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setCurrentWeek(parseWeekId(e.target.value));
  };

  const handleSlotClick = (date: Date, slotIndex: number) => {
    const dateStr = date.toISOString();
    
    // Nếu ô đã được tích chọn (đang pending) thì bỏ tích
    ...
    // Kiểm tra xem ô này đã có ai đặt trên Database chưa
    const existingBooking = bookings.find(b => b.slotIndex === slotIndex && isSameDay(new Date(b.date), date));
    
    // Quyền click: Nếu ô trống HOẶC là ô của chính mình HOẶC mình là Admin
    ...
    if (canEdit) {
      ...
      // Lấy nội dung cũ (nếu có) để Admin/User sửa hoặc xóa
  ...
  const handleSave = async () => {
    ...
    try {
      for (const p of pendingBookings) {
        // Gửi nội dung lên, nếu Admin xóa trắng content -> API sẽ tự động xóa bản ghi trong DB
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
            ...
            {currentUser ? (
              ...
              <div className="flex items-center space-x-2">
                <button 
                  ...
                  className="flex items-center space-x-1 bg-gray-100 hover:bg-gray-200 text-gray-800 px-3 py-2 rounded-md text-sm font-medium transition-colors"
                >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
          <div className="flex space-x-2">
            ...
            <button 
              ...
              className="px-3 py-2 border border-indigo-300 shadow-sm text-sm font-medium rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100"
            >
              Hiện tại
            </button>
            <button 
        ...
        <div className="bg-white shadow ring-1 ring-black ring-opacity-5 rounded-lg overflow-hidden">
          <div className="grid grid-cols-8 border-b border-gray-200 bg-gray-50">
            <div className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">
              Thời gian
            </div>
          ...
          <div className="divide-y divide-gray-200 bg-white">
            {TIME_SLOTS.map((slotName, slotIndex) => (
              <div key={slotIndex} className="grid grid-cols-8">
                <div className="py-4 px-4 text-sm text-gray-500 font-medium border-r border-gray-200 flex items-center bg-gray-50">
                  {slotName}
                </div>
                {daysOfWeek.map((day, i) => {
                  ...
                  return (
                    <div 
                      key={i} 
                      className={`p-2 border-r border-gray-200 last:border-r-0 min-h-[120px] relative transition-colors ${isPending ? 'bg-green-50' : 'hover:bg-gray-50'}`}
                    >
                      ...
                      <div className="mb-2">
                        <label className="flex items-center space-x-2 cursor-pointer">
                          <input 
                            type="checkbox" 
                            checked={isPending}
                            onChange={() => handleSlotClick(day, slotIndex)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            <textarea
              ...
              className="w-full h-32 p-3 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 resize-none"
            />
      ...
      <AuthModals 
        ...
        openLogin={() => { setIsRegisterModalOpen(false); setIsLoginModalOpen(true); }}
        openRegister={() => { setIsLoginModalOpen(false); setIsRegisterModalOpen(true); }}
        onSuccess={(user: {userId: string, username: string}) => {
          setCurrentUser(user);
          localStorage.setItem('currentUser', JSON.stringify(user));
        }}
      />

      {/* MANAGE USERS MODAL CHO ADMIN */}
      {isManageUsersOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center p-4 border-b bg-gray-50">
              ...
              <button onClick={() => setIsManageUsersOpen(false)} className="text-gray-400 hover:text-gray-500">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-4 max-h-[60vh] overflow-y-auto">
              {usersList.length === 0 ? (
                <p className="text-center text-gray-500 py-4">Đang tải danh sách...</p>
              ) : (
                <ul className="divide-y divide-gray-200">
                  {usersList.map(u => (
                    <li key={u._id} className="py-3 flex justify-between items-center">
                      <div className="flex flex-col">
                        <span className={`text-sm font-medium ${u.username === 'hiep14082005' ? 'text-indigo-600' : 'text-gray-900'}`}>
                          {u.username} {u.username === 'hiep14082005' && '(Quản trị viên)'}
                        </span>
                        <span className="text-xs text-gray-500">
                          Tham gia: {format(new Date(u.createdAt), 'dd/MM/yyyy')}
                        </span>
                      </div>
                      {u.username !== 'hiep14082005' && (
                        <button 
                          onClick={() => handleDeleteUser(u._id)}
                          className="text-xs font-semibold text-red-600 hover:text-white border border-red-600 hover:bg-red-600 px-3 py-1.5 rounded transition-colors"
                        >
                          Xóa tài khoản
                        </button>
                      )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 relative">
        <button onClick={isLogin ? closeLogin : closeRegister} className="absolute top-4 right-4 text-gray-400 hover:text-gray-500">
          <X className="w-5 h-5" />
        </button>
